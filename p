...
14) which of the following is relavent to transient ---- 
	(serialization)
...
23) which is the function works on map---
	(put())
24) which elementsare used to identify a dependent artifact in a pom.xml file? 
...
36) thread t = new Thread() System.out.println(t.isAlive())	
	false
37) what are the eligible metods for interfaces? 
